 ...
 * the code disagree, so this has to be re-run when a page starts or stops
 * using AI.
 */
...
const rows = caps
  .map(
    (c) =>
      `  { route: ${JSON.stringify(c.route)}, needsAI: ${c.needsAI}, usesBrowser: ${c.usesBrowser} },`,
  )
  .join("\n");

const tools = caps.filter((c) => c.route.startsWith("/tools/"));
...
// Of those, ${tools.length} are /tools/* · ${tools.filter((c) => c.needsAI).length} need AI.
...
console.log(`wrote ${caps.length} routes to ${path.relative(process.cwd(), dest)}`);
